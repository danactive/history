...
tape('Log', { skip: false }, (describe) => {
  ...
  const createOptionskHandler = (assert, testMessage) => ({
    callback: (message) => {
  ...
  describe.test('* Operational message', (assert) => {
    ...
    const logger = lib.createLogger(moduleName, createOptionskHandler(assert, testMessage));
  ...
  describe.test('* Console logs', (assert) => {
    ...
    const logger = lib.createLogger(moduleName);
